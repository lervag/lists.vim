...
function! lists#debug#stacktrace(...) abort " {{{1
  "
  ...
  for l:func in l:stack
    ...
    if l:name =~# '\v(\<SNR\>|^)\d+_'
      ...
      let l:filename = substitute(
            \ lists#u#command('scriptnames')[l:sid-1],
            \ '^\s*\d\+:\s*', '', '')
    else
      let l:func_name = l:name =~# '^\d\+$' ? '{' . l:name . '}' : l:name
      let l:func_lines = lists#u#command('verbose function ' . l:func_name)
    ...
    if filereadable(l:filename)
      ...
      if l:name =~# '^\d\+$'
        let l:lnum = 0
        let l:output = lists#u#command('function {' . l:name . '}')
