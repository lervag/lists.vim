...
function! lists#parser#get_at(lnum) abort "{{{1
...
" }}}1
function! lists#parser#get_current() abort "{{{1
  return s:get_list()
endfunction

" }}}1
function! lists#parser#get_previous() abort "{{{1
...
function! s:init_item_types() abort " {{{1
  ...
  for l:t in map(glob(s:glob_path, 0, 1), "fnamemodify(v:val, ':t:r')")
    ...
    let l:item_type = lists#item#{l:t}#new()
